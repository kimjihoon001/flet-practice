...
하나의 프로그램 내에서 사무용 계산기와 공학용 계산기를 자유롭게 전환할 수 있도록 설계되었으며, 직관적인 UI와 안정적인 수식 처리 로직을 기반으로 다양한 연산을 지원한다.
특히 문자열 기반 수식 처리 구조(`expression` / `eval_expression` 분리)를 통해 정확한 계산 수행과 사용자 친화적 인터페이스를 동시에 구현하였다
